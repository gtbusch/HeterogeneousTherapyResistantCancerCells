...
new_path <- file.path(parent_path, "RawData", "BulkRNASeq_Run2")

data_path <- file.path(parent_path, "ExtractedData", "BulkRNASeq_Run2")
...
for (subfolder in subfolders) {
  ...
  files_in_subfolder <- list.files(subfolder, pattern = "^GTB_07102024_Step1_TxImport_(metadata|myTPM|myCounts)\\.rds$", full.names = TRUE)
...
# Task 1: Manipulate the myTPM data frame such that 
...
saveRDS(task1_annotatedTPM, file.path(data_path, file = "bulkrnaseq_run2_tpm_plusmetadata.rds"))

saveRDS(task1_annotatedCounts, file.path(data_path, file = "bulkrnaseq_run2_counts_plusmetadata.rds"))

write_tsv(task1_annotatedTPM, file.path(data_path, "bulkrnaseq_run2_tpm_plusmetadata.tsv"))

write_tsv(task1_annotatedCounts, file.path(data_path, "bulkrnaseq_run2_counts_plusmetadata.tsv"))
